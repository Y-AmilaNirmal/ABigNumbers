...
ABigNumbers::ABigNumbers(LiquidCrystal_I2C *lcd)
{
    _lcd = lcd;
}

void ABigNumbers::begin()
{
...
void ABigNumbers::displayLargeInt(int n, byte x, byte y, byte digits, bool leading)
{
...
void ABigNumbers::clearLargeNumber(byte x, byte y)
{
    _lcd->setCursor(x, y);
    _lcd->print(F("   "));
...
void ABigNumbers::displayLargeNumber(byte n, byte x, byte y)
{
    switch (n)
    {
    case 0:
